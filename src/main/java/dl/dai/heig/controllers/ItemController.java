...
public class ItemController {
  ...
  // As the ID for items will never be All Items, it is safe to reserve the value All Items for all
  // items
  private final String RESERVED_ID_TO_IDENTIFY_ALL_ITEMS = "All Items";

  public ItemController(
      ConcurrentHashMap<String, Item> items, ConcurrentHashMap<String, LocalDateTime> itemsCache) {
    this.items = items;
  ...
  // Cache is not possible here, because we don't have an id to check if the user already has it in
  // their cache.
  ...
  public void getRandomOneByPool(Context ctx) {
    ...
    Map<String, Item> filtered =
        ItemFilter.filterItems(items, ItemFilter.FilterType.ITEM_POOL, pool);
  ...
  public void getRandomOneByQuality(Context ctx) {
    ...
    int d6 = rand.nextInt(items.size());
    Map<String, Item> filtered =
        ItemFilter.filterItems(items, ItemFilter.FilterType.QUALITY, quality);
    List<String> keysAsArray = new ArrayList<>(filtered.keySet());
